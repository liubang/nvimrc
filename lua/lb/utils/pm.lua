...
local config = function()
  ...
  g['dein#install_log_filename'] = string.format('%s/dein.log', g.cache_path)
end

local init_dein = function()
  if not vim.fn.has('vim_starting') then
    return
  end
  if not o.runtimepath:match('/dein.vim') then
    local dein_dir = string.format('%s/repos/github.com/Shougo/dein.vim', repo_path)
    if 0 == vim.fn.isdirectory(dein_dir) then
      os.execute(string.format('git clone https://github.com/Shougo/dein.vim %s', dein_dir))
    end
