...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(event)
    ...
    -- stylua: ignore
    vim.keymap.set("n", "<Leader>ee", function() vim.diagnostic.open_float(nil, { scope = "line" }) end, bufopts)
    local winopts = { height = 15, width = 100, max_height = 20, max_width = 140 }
    -- stylua: ignore
    vim.keymap.set("n", "<C-k>", function() vim.lsp.buf.hover(winopts) end, bufopts)
    -- stylua: ignore
    vim.keymap.set("n", "<C-h>", function() vim.lsp.buf.signature_help(winopts) end, bufopts)
